<?php 
	...
	class Audio extends MY_Controller {
		...
		public function upload(){ 
            ...
            $where = array('loai' => 2);
            $data['data']['dropdown_bai'] = $this->chude_model->dropdown($where);
            // print_r($data);die;
            $data['template'] = 'audio/add';
        ...
        public function view() {
            ...
            $data['data']['table'] = $this->audio_model->view();
            $data['template'] = 'audio/view';
        ...
        public function delete() {
            $id_bai = $this->encrypt->decode(base64_decode($this->input->get('id_bai')));
            $id_theloai = $this->encrypt->decode(base64_decode($this->input->get('id_theloai')));
            $param['where'] = array('id_bai' => $id_bai, 'id_theloai' => $id_theloai);
            $flag = $this->audio_model->delete($param);
            $this->session->set_flashdata('message', $flag);
            redirect('audio/view');
            $this->output->enable_profiler(true);
        }

        public function json_encode_audio($id_bai, $id_theloai) {
            $param['where'] = array('id_bai' => $id_bai, 'id_theloai' => $id_theloai);
            $data = $this->audio_model->view($param);
